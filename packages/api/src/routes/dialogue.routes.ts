...
export async function dialogueRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Params: { projectId: string }; Body: unknown }>(
    ...
    async (request, reply) => {
      ...
      const result = await captureProjectFile(projectId, parsed.data, undefined, request.userId ?? undefined);
  ...
  fastify.post<{ Params: { projectId: string; fileName: string }; Body: unknown }>(
    ...
    async (request, reply) => {
      ...
      const result = await enrichProjectFile(projectId, fileName, parsed.data, request.userId ?? undefined);
  ...
  fastify.post<{ Params: { projectId: string; fileName: string }; Body: unknown }>(
    ...
    async (request, reply) => {
      ...
      const result = await correctProjectFile(projectId, fileName, parsed.data, request.userId ?? undefined);
      return reply.send(result);
    }
  );
}
