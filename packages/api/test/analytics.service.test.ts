...
describe('analytics.service track()', () => {
  ...
  it('uses userId as distinct_id when authed, keeping session_id as a property (WIC-822)', async () => {
    // Authed events must collapse onto a stable per-user identity so Dashboard C
    // (retention, baseline §2.3) can compute Return Upload Rate / Uploads-per-user.
    setEnv({
      ANALYTICS_SINK: 'posthog',
      POSTHOG_API_KEY: 'phc_test',
      POSTHOG_HOST: 'https://ph.example.com',
    ...
    // distinct_id is the user, not the session...
    expect(payload.distinct_id).toBe('user-42');
    // ...but session_id survives as a property for session-level analysis.
    expect(payload.properties.session_id).toBe('sess-1');
  });

  it('falls back to session_id as distinct_id for anonymous (no userId) events (WIC-822)', async () => {
    setEnv({
      ANALYTICS_SINK: 'posthog',
      POSTHOG_API_KEY: 'phc_test',
      POSTHOG_HOST: 'https://ph.example.com',
    ...
    // No 4th arg — pre-login flow stays session-scoped and later aliases onto
    // the user via the client identify() call.
    await track('resume_upload_started', { file_type: 'pdf' }, 'sess-anon');

    const payload = JSON.parse((fetchSpy.mock.calls[0][1] as RequestInit).body as string);
    expect(payload.distinct_id).toBe('sess-anon');
    expect(payload.properties.session_id).toBe('sess-anon');
  });
