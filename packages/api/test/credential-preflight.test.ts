...
function fakeFetch(
  routes: Array<{ match: string; status?: number; throws?: string }>
): typeof fetch {
...
describe('credential-preflight', () => {
  ...
  describe('HTTP provider checks', () => {
    ...
    it('cloudflare: unauthorized token surfaces as fail (WIC-869 class)', async () => {
      const d = deps({
        env: { CLOUDFLARE_API_TOKEN: 'cf-dead' },
        fetch: fakeFetch([{ match: 'api.cloudflare.com', status: 401 }]),
      });
  ...
  describe('aggregation and formatting', () => {
    it('runPreflight is not ok when any provider fails', async () => {
      const d = deps({
        env: { ANTHROPIC_API_KEY: 'good', CLOUDFLARE_API_TOKEN: 'dead' },
        fetch: fakeFetch([
          { match: 'api.anthropic.com', status: 200 },
          { match: 'api.cloudflare.com', status: 403 },
        ]),
      });
