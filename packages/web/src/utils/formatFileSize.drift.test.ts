 ...
 * byte count. This is the guard that keeps it the only one.
 *
 ...
 * across two shapes: its sibling at `:48` computes the same value into `sizeMB` and
 * does not match, because the unit literal sits on a different line. Do not try to
 * express "limit renderer" in the pattern; pin the line.
 *
