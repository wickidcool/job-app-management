...
export function QuickReferenceExport({
  ...
  return (
    // Mount-controlled: the parent renders this only while it should be open.
    <Dialog.Root
      open
      onOpenChange={(next) => {
        if (!next) onClose();
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg max-w-4xl w-[calc(100%-2rem)] max-h-[90vh] overflow-hidden flex flex-col z-50"
          {...focusRestore}
        >
          {/* Header */}
