...
- **Cleartext requests are redirected** — `httpsRedirect()` (`packages/api/src/middleware/security.ts`) turns away any request that arrived over HTTP before a handler runs: `301` for `GET`/`HEAD`, `308` for methods with a body so the retry keeps its method and payload. The client scheme is read from Cloudflare's `cf-visitor`, falling back to `x-forwarded-proto` and then the request URL. Loopback hosts are exempt, so `npm run dev:api` / `wrangler dev` / Vitest stay on plain HTTP.
...
- **Known residual gap** — the Worker-side redirect only covers paths the Worker actually sees (`/api/*`, `/health`, SPA deep links). A first-contact request to `/` or `/assets/*` over port 80 is still answered by the asset router in cleartext; HSTS closes it for every subsequent visit, but fully closing first contact needs a Cloudflare zone action (extend Always-Use-HTTPS / add a redirect rule covering the app host) rather than a code change.
