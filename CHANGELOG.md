...
- **Accessibility fixes in the same pass.** The "Requested path" label was `neutral-500` on a `neutral-100` panel, which measures **4.39:1 — a WCAG AA failure** for normal text; dropping the panel and moving the label to `neutral-600` on `neutral-50` takes it to 7.2:1. The `Error 404` eyebrow and keyboard hint moved `neutral-500` → `neutral-600` for the same reason (4.63:1 → 7.2:1, i.e. off the AA borderline).
...
- **Copy remains first-pass, pending Copywriter/Editor.** Strings are isolated in a single `COPY` block at the top of `NotFound.tsx` so a wording revision is a one-hunk diff that cannot disturb layout.
