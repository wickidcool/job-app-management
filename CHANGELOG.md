...
- **Coverage** — `packages/web/e2e/not-found.spec.ts` (5 Playwright specs, mock auth, no backend required): the 404 renders on an unmatched path, names that path, keeps app chrome and returns to the dashboard, does not swallow a real route, and the unauthenticated case still redirects to `/login`.
