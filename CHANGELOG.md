...
- **The AC-10 bypass is documented as scoped, with the reasoning that makes it stick.** A three-row decision table gives the shipped precedence — completed, then engaged, then the history probe — followed by the argument for that order: the resume-upload step *produces* a resume, so a history-first check would eject a new user from the flow at the moment its own step succeeded. Recording the rule without the argument would invite the "simplification" back.
