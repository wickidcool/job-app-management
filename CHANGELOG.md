...
- **`POST /api/catalog/generate-diff` was orphaning every diff it wrote.** `processCatalogChange` reads the owner off `event.metadata.userId` (the shape `resume.service.ts` uses when it enqueues), and `generateDiff` passed no metadata at all — so the `catalog_diffs` row, and every catalog row auto-applied alongside it, landed with `user_id` null, invisible ever after to `listDiffs`, `getDiff` and `applyDiff`, all of which are scoped. The caller is now threaded through.
