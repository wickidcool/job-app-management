...
- **Documents are read scoped too** — `POST /api/catalog/generate-diff` takes `sourceId` from the request body, so a caller naming someone else's resume ULID had that resume extracted into catalog rows. The document read is now owner-scoped, so the run yields nothing instead. That endpoint still performs work on a document the caller does not own rather than rejecting it; tracked separately as WIC-1414.
- **Verified against real Postgres, and against the reverted fix** — the suite runs on PGlite, not a hand-rolled `db` stub: a stub resolves whatever rows it was primed with regardless of the predicate it is handed, which is exactly how WIC-1373 shipped two tenancy assertions that passed *with* the bug. Only a real planner distinguishes `WHERE tag_slug = $1` from `WHERE tag_slug = $1 AND user_id = $2`. Reverting the fix fails 10 of the 12 tests; the two survivors are single-tenant controls, which by construction cannot tell the two predicates apart. The `job_fit_tags` coverage was added last and was not redundant — `RESUME_TEXT` trips no `JOB_FIT_PATTERN`, so before it, reverting both jobFit predicates left the whole suite green.
