...
- **Open caveat — canonical-domain routing** — the canonical production domain `https://careerpin.app/api/*` currently returns the SPA shell (`200 text/html`), i.e. it is **not yet routed to the `jobtrail` Worker**. The SPA's API base defaults to same-origin `/api` (`packages/web/src/services/api/index.ts`), so until a Cloudflare custom-domain/route attaches `careerpin.app/api/*` to the Worker, server-side events driven by real users on the canonical domain are **not confirmed to be reaching the PostHog sink**. PostHog-side confirmation of the 3 server events landing in Live Events is tracked under WIC-964 (blocked on PostHog console access). Client events are unaffected by this route (they post directly to the PostHog host).
