...
- **The bypass is scoped to users who have never engaged with the flow, not to anyone with a resume.** The naive reading of AC-10 — count resumes and applications, return false if either is non-zero — introduces a worse bug than it fixes: onboarding step 3 *is* the resume upload, so a genuine new user would be ejected from the flow at the moment the flow's own step succeeded. `shouldShowOnboarding` now consults history only when the status row cannot answer: no row at all, or a pristine one still on `welcome` with nothing completed or skipped. A row that shows any engagement is mid-flow and is honoured as-is.
...
- **Existence probes, not counts** — `select id … limit 1` on each table, resume first, short-circuiting the second round trip. A completed user still costs exactly one query; the probes only run for users the status row cannot settle.
- **The two WIC-1354 tests that pinned the gap are replaced by tests that pin the fix.** The `it.fails` AC-10 case is now a passing assertion, and the sentinel that asserted "consults only `onboarding_status`" — true of the defect — is now a per-scenario query-count assertion: 1 select for a completed user, 1 for a mid-flow user (proving history is not consulted mid-flow), 2 when a resume short-circuits, 3 for a user with no row and no history. Both halves of the fix were reversal-checked: removing the AC-10 guard fails 4 tests, removing the mid-flow scoping fails 2.
