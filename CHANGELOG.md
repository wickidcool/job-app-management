...
- **That correction is enforced, not just written down.** An annotation is a plausible edit — a lint preference, a "be explicit" review note, an IDE quick-fix — and it disables the exhaustiveness check with a clean build and no other signal. `_VERDICT_TIERS_KEEPS_ENTRY_LITERALS` now fails compilation if the first entry's `tier` widens to the whole `VerdictTier` union, which is exactly what an annotation does to every entry at once. It names no tier literal, so reordering or renaming tiles leaves it alone.
- **Verified in six directions, not one.** Unmodified tree: exit `0`, no false positive. Adding `'perfect_fit'`: fails naming `"perfect_fit"`. Deleting the `low_fit` tile: fails naming `"low_fit"` — the protection the old `Extract` did provide is retained. Giving `'unscored'` a tile: rejected at the constraint, which the filter form also missed. Annotating `VERDICT_TIERS` **with** `as const satisfies` intact: now fails on the annotation guard (before this change, exit `0`). Annotating it **without** `satisfies`: same failure. Reordering the tiles: still exit `0`. `packages/api/test/fit-tier-blurbs.test.ts` re-reads these entries out of the source by regex and still passes, so the cross-package blurb check is unaffected.
