...
- **Regression coverage** — `packages/web/e2e/modal-focus-projects.spec.ts`, 8 Playwright tests against the create-project dialog, the one of the five that both autofocuses its own field and has parent state to reset. Two negative controls confirm the coverage is load-bearing: reverted to the pre-migration component, 8 of 8 fail; keeping the Radix migration but dropping only the `useDialogFocusRestore` handlers, exactly the 2 focus-restore tests fail.
