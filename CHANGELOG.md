...
- **`as const` is load-bearing.** An explicit `ReadonlyArray<{ tier: VerdictTier; … }>` annotation widens each entry's `tier` to the whole union, which would make `(typeof VERDICT_TIERS)[number]['tier']` vacuous and the guard useless. `as const satisfies` keeps the entry literals while still constraining the shape.
- **Verified in four directions, not one.** Unmodified tree: exit `0`, no false positive. Adding `'perfect_fit'`: fails naming `"perfect_fit"`. Deleting the `low_fit` tile: fails naming `"low_fit"` — the protection the old `Extract` did provide is retained. Giving `'unscored'` a tile: rejected at the constraint, which the filter form also missed. `packages/api/test/fit-tier-blurbs.test.ts` re-reads these entries out of the source by regex and still passes (3/3), so the cross-package blurb check is unaffected.
