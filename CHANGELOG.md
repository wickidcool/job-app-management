...
- **The test double now binds results to the table read, not to the call index (WIC-1371).** Counting selects proves *how many* tables were consulted but not *which*, and the stub's `from`/`where` discarded their arguments — so a probe of the wrong table simply drew the next queued row and the suite stayed green. `stubDb` is now keyed by table name: a declared `[]` means "probed, found nothing", an undeclared table throws on read, and every `where` argument is rendered to `column op ?` plus its bound values so a predicate can be asserted directly. Assertions moved from `select` call counts to the actual read sequence, e.g. `['onboarding_status', 'resumes']`. Measured against four mutations of `hasExistingWork`: swapping the two probe tables, probing `resumes` twice and never `applications`, and scoping the resume probe by `id` instead of `user_id` all passed 11/11 under the old stub and all fail now; dropping the `.where()` outright was already caught, but as a `TypeError` rather than as a missing predicate. The scoped-predicate case is the one that matters next — a probe carrying `and(user_id = ?, uploaded_at < ?)` versus a bare `user_id = ?` was indistinguishable before and is now a direct equality assertion.
