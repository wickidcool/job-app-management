...
**1. Head drift — disclosed, and immaterial.** The ask was filed `2026-08-30T00:01:41Z`; PR #148's head then advanced to `a3e833d0` at `04:44:15Z`, **4h43m after the ask**. A `request_board_approval`/ask records no head SHA, so nothing would have flagged this. I diffed it: `a3e833d0` touches **one file** (+149/-42) and **no runtime code** — docs/architecture/adr/ADR-007-workers-subrequest-budget.md, which lives on PR #148's branch and **not on `main`**, so it is named here as prose rather than cited as a path you can open. Per approval-head-drift rule 2 that is a docs/posture delta — the standing ask stands, no withdraw-and-recreate needed. This comment is the drift record that rule requires.
