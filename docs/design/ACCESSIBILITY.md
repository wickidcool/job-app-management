...
>
> **Resolved by `586712c2`, and the resolution is the reason the last row of the table is worth its weight.** `routeOutlineRoutes.tsx` mounts the **real** `<ApplicationsList />`, so `/applications` is now judged as the route rather than as a fixture, and the skip described above would fail the build in all four branches. **The `KanbanBoard.test.tsx` fixture is unchanged and still does not match the page** — that was never the thing to fix, and "nothing checks that it does" is what stopped being true. WIC-1834 is closed.
>
