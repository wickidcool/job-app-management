...
> **Enforcement status: partial, and far narrower than this document's scope.** Measured against `main` @ `2c3b1ba3` (2026-09-01). **Five** checks fail the build, all of them steps in the **`Lint & Test`** job of `.github/workflows/deploy.yml`. The first two arrived with `f3ed4e3` and are new since this note last read *three*:
>
> | check | the command that fails the build | what it actually covers |
> | --- | --- | --- |
> | `eslint-plugin-jsx-a11y` at `flatConfigs.strict` (WIC-1483) | `npm run lint` | **24 rules at `error`** — a new violation of any of them fails outright — plus **8** rules held at `warn` beneath a `--max-warnings 47` ceiling, and **2** deliberately `off`. **Per-file only**, so it does not reach SC 1.3.1; see below. |
> | `src/test/jsxA11yBaseline.test.ts` (WIC-1483) | `npm run test` | The ratchet under that ceiling. The 47 pre-existing findings are pinned per file **and** rule in *both* directions, so a new one fails and a *fixed* one also fails until the baseline is lowered to match. It also cross-checks the `--max-warnings` numbers in `lint` and `lint:fix` against its own total, and asserts the resolved config is `strict` rather than `recommended`. |
...
>
> **Everything else in this document is still guidance a reviewer checks by hand.** ~~`npm run lint` carries **no accessibility rules at all**: at `9ec6309`, `packages/web/eslint.config.js` loads only `js`, `typescript-eslint`, `react-hooks`, `react-refresh` and `prettier`, and none of `eslint-plugin-jsx-a11y`, `axe-core`, `pa11y` or a Lighthouse budget is a dependency.~~ **Half of that is withdrawn as of `f3ed4e3`, and the half that survives is the more important half.** `eslint-plugin-jsx-a11y@^6.10.2` *is* a dependency of `@wic/web` and the config *does* extend it, so the first clause is now simply false — but **`axe-core`, `pa11y` and a Lighthouse budget are still absent from every `package.json` in this repo, and `deploy.yml` still has no step that renders a page and inspects it.** That is the difference between a *static* check and a *rendered* one, and it is why nothing below about colour contrast or screen-reader behaviour has moved. The [Testing Checklist](#testing-checklist) remains the whole process for every criterion outside the table above, and **no box in it is automated** — the boxes and the table describe different things, and landing a linter did not tick any of them.
>
> **SC 1.3.1 is not covered site-wide, and the lint layer that has now landed does not cover it.** Heading order is a property of the *composition* of a page and the components it mounts, so it is structurally invisible to any per-file rule — WIC-1483 measured that `eslint-plugin-jsx-a11y` would have caught **none** of the 16 heading skips it found, and `A11Y_ENFORCEMENT_RULING.md` §4.2 records the same limit independently. This is not an inference from the plugin's reputation: both halves of the shipped layer say it about themselves, in `BASELINED_RULES`' header comment in `eslint.config.js` (*"structurally blind to heading-order defects that exist only in the composition of a page and the component it mounts"*) and in `jsxA11yBaseline.test.ts`'s own scope limit (*"This suite does NOT cover WCAG SC 1.3.1"*). **Do not credit `jsx-a11y` with SC 1.3.1** — landing it changed nothing in this paragraph except the tense. Of the two heading checks that do exist, the first is blind to expression-built headings (**11 of 33 `<h1>` and 11 of 42 `<h2>`** were already invisible to it at `6911bcb`), and the second covers 2 components against **29 page components and 32 distinct route paths**. Take those two denominators from the source, not from a raw glob: `packages/web/src/pages/*.tsx` is 33 files, but four of them are tests (`ApplicationsList.statusParam`, `CoverLetterNew`, `NotFound`, `OutreachNew`), and `App.tsx` declares 34 `<Route>` elements that resolve to 32 distinct `path=` values, two of which (`*`, `/*`) are catch-alls.
>
...
>
> **Layer 1 of the ruling has landed. Layers 2 and 3 have not — do not read this as "enforced".** This replaces the "decided but not landed" note that stood here until 2026-09-01, on the trigger that note named for itself. WIC-1192 ruled on 2026-08-30 (`A11Y_ENFORCEMENT_RULING.md`, `ee6c217`) that the repo adopts `jsx-a11y` at `strict`/`error` behind a frozen baseline, **plus `axe-core` hosted in the vitest + RTL harness** rather than in Playwright E2E. The first half shipped in **`f3ed4e3`**; the second has not been started.
>
> What is actually on `main`, re-measured at `2c3b1ba3` rather than taken from the merged PR's description: `packages/web/eslint.config.js` extends `jsxA11y.flatConfigs.strict` and restores `anchor-ambiguous-text` at `error`, resolving to **24 rules at `error`, 8 at `warn`, 2 deliberately `off`**; the frozen baseline is **47 findings across 22 files over 8 rules**; `npm run lint` reports `47 problems (0 errors, 47 warnings)` and exits 0 against its `--max-warnings 47` ceiling. Those counts are not restated here on trust — `jsxA11yBaseline.test.ts` asserts every one of them against the *resolved* config, which is the only copy that cannot rot, and the per-rule tallies in `BASELINED_RULES` sum to the same 47 independently.
>
> **What remains unverified, which is most of this document:**
>
> - **SC 1.3.1 is untouched by this.** See the paragraph above — the shipped layer says so about itself in two places.
> - **The 47 are baselined, not fixed.** They are real defects, owned by **WIC-1589**, whose PR **#292** is open and unmerged as of 2026-09-01. A baseline is a promise not to get worse; nothing here got better.
> - **Layer 2 (rendered per-route, per-render-branch heading outline) is not on `main`.** WIC-1675's card reads `done`, and its PR **#299** is nevertheless **open and unmerged** — verified on GitHub, not from the card. A `done` status is not a merge; do not describe layer 2 as shipped on the strength of it.
> - **Layer 3 (`axe-core` in the vitest + RTL harness) does not exist.** No `axe-core`, `pa11y` or Lighthouse budget appears in any `package.json`. Everything in this document that needs a *rendered* page — contrast, screen-reader behaviour, live-region announcements — is exactly as unmeasured as it was before `f3ed4e3`.
>
> **A lint rule set plus two heading checks is not WCAG 2.1 AA**, which is the sentence the note this replaced asked its replacement to carry. Citation written under **WIC-1584**; updated under **WIC-1910**.
...
**None of these is wired up**, and that is still true after `f3ed4e3`. These three boxes have been unchecked since this document was written; they describe tools someone could run, not a pipeline that runs them. An unchecked box here means "nobody has done this", not "this is queued" — see the enforcement-status note at the top of this document. **Landing the `jsx-a11y` config did not tick any of these three**, and the reason is the distinction that note draws: `jsx-a11y` is a *static* per-file lint, while all three tools below render a page and inspect the result. `axe-core`, `pa11y` and Lighthouse remain absent from every `package.json` in the repo.
...
> under **WIC-1483** and **merged 2026-09-01 in `f3ed4e3`**.
>
> ⚠️ **This note predicted that "boxes 2 and 4 become machine-checkable" once that config reached
> `main`, and said the hand counts below should then be deleted. The config has landed and the
> prediction is half wrong, so the counts stay.** Measured at `2c3b1ba3`:
>
> - **Box 2 gained a machine check, but not one that measures what the box counts.**
>   `label-has-associated-control` is live at `warn` with **19** findings across 7 files, pinned by
>   the baseline test. Box 2's own figure is **28 of 98** form controls, because it counts
>   *controls lacking an accessible name* while the rule counts *`<label>` elements lacking an
>   association*. Deleting the hand count in favour of the lint count would silently narrow the
>   claim to a smaller one, so both are stated.
> - **Box 4 gained nothing.** The rule that would cover it, `control-has-associated-label`, is one
>   of the **two deliberately `off`** — `eslint.config.js` turns it off with the note that its
>   findings "belong with WIC-1589's work, not smuggled into this change." Run at `error` it
>   reports **3** findings, in `FilterPanel.tsx`, `ResumeUpload.tsx` and `InterviewPrepPage.tsx` —
>   and two of those three are precisely the controls box 4 names by hand. **The one rule that
>   would have automated box 4 is the one that is switched off**, so its hand count is still the
>   only record that exists.
>
> The general lesson is the one this list was rewritten to teach: a prediction about what a future
> commit will make checkable is itself a claim about the future, and rots the same way a PR number
> does. Re-run the measurement before deleting anything on the strength of one.
>
