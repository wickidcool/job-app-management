...
>
> **What remains unverified.** Heading order plus a lint rule set **plus axe** is not WCAG 2.1 AA, and nothing above changes that — the ruling's own §7 says so, and adopting the second half of it does not make the pair complete. Read the section above as "the ruled mechanism is now fully landed", never as "this tree is AA". The 47 lint findings are **frozen, not fixed** — that is **WIC-1589** — and so are the 26 axe findings, which are **WIC-1942**. Rendered per-route, per-render-branch outline coverage **has landed**: WIC-1675 merged as `#299` and is the commit `main` points at, so the four skips described above are no longer invisible to the table; that clause previously read "until it lands" and is corrected here. Citation written under **WIC-1584**; revised under **WIC-1902**, and again under **WIC-1926**.
