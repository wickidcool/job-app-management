...
> | `findOutlineSkips` / `getOutline` (`src/test/headingOutline.ts`, WIC-1571) | `npm run test` | **Rendered** heading-outline skip assertions. The helper itself; the row below is now its largest consumer. Directly asserted against **three application components** — `KanbanBoard.test.tsx`, `CoverLetterPreview.test.tsx` and `pages/CoverLetterNew.test.tsx`, which mounts the real `<CoverLetterNew />`. **The "exactly two" this row used to claim was wrong from 2026-08-27**: `b4b87c66` added the third under WIC-1571 — the same card this row cites — and the enumeration was never revisited. Its remaining importers are `src/test/headingOutline.test.tsx` (the helper's own unit test against deliberate fixtures, which enforces nothing about the app) and `src/test/routeOutlineHarness.tsx`. |
...
> | `src/test/routeOutline.render.test.tsx` + `routeOutline.source.test.ts` (WIC-1675, landed `586712c2` in PR #299) | `npm run test` | **The site-wide SC 1.3.1 check.** Mounts every route under jsdom in all four API branches — `loading`, `error`, `empty`, `loaded` — and judges the outline a screen reader would build. **No heading level is skipped on any route on any branch, with no allowlist and no exemptions.** The weaker missing-`h1` half *is* allowlisted: 25 (route, branch) pairs are inventoried in `MISSING_H1`, pinned at that length and guarded in both directions, so the list can only shrink. It also asserts its own route table is complete against `App.tsx`, so a new route cannot silently drop out of the sweep, and pins the volume it measured (30 routes × 4 branches = 120 pairs) so a harness failure goes red rather than quiet. |
> | `src/test/route-title-coverage.test.ts` + `RouteTitle.test.tsx`, `useDocumentTitle.test.tsx`, `constants/title.test.ts` (WIC-1089, landed `7f8646d2` in PR #336) | `npm run test` | **The site-wide SC 2.4.2 (Page Titled, Level A) check.** Until it landed, all 30 rendered routes shared the single static `<title>` in `index.html`, so the tab, the history entry, the bookmark name and the window-switcher entry were the same string for every screen. The coverage test reads `App.tsx` as **raw source** and reconciles its route table against `constants/title.ts` **in both directions**: every declared `path=` is accounted for by exactly one titling strategy — static, hook-titled, or redirect — and no titled path is absent from `App.tsx`. So a `<Route>` added without a title fails CI, and a deleted route cannot leave a dangling title behind. It also reads the redirect classification out of `App.tsx`'s own `element={<Navigate` rather than trusting the constant, asserts the two redirects carry no title, guards its parse against matching nothing, and pins that `<RouteTitle />` is mounted in the shell — the one assertion that the parts are wired together rather than merely correct in isolation. **What it does not cover:** that each string *describes* its screen. That is the convention's rule (a route's title is its `<h1>` verbatim, `ROUTE_TITLE_CONVENTION.md` §0.3) and it is checked by review, not by CI — and the seven hook-titled routes are swept only for *calling* the hook, never for the string they pass. `e2e/not-found.spec.ts` pins the real browser's `document.title` on navigation and back-navigation, em dash included, and `docs/design/route-title-table-audit.py` — wired into both `docs-audit.yml` and `deploy.yml` — fails the build when `ROUTE_TITLE_CONVENTION.md` §5's documented table drifts from `App.tsx`, printing the reconciliation as it goes (**30 documented routes, 32 in `App.tsx`, 2 redirect-only**). |
>
...
>
> **SC 1.3.1 is still not covered site-wide, but its heading-order component now is — and the lint layer is not what covers it.** Read the two apart: since `586712c2` no route can skip a heading level on any branch, which is the part of SC 1.3.1 this document has tracked longest; the rest of the criterion — programmatic labels, list and table semantics, relationships conveyed only visually — has no site-wide check, and the boxes in the [Testing Checklist](#testing-checklist) are still the whole process for it. Heading order is a property of the *composition* of a page and the components it mounts, so it is structurally invisible to any per-file rule — WIC-1483 measured that `eslint-plugin-jsx-a11y` would have caught **none** of the 16 heading skips it found, and `A11Y_ENFORCEMENT_RULING.md` §4.2 records the same limit independently. The shipped config says so in its own source: `eslint.config.js:33` carries a `NOTE (WIC-1483)` that `jsx-a11y` is per-file and therefore structurally blind to this. **Do not credit `jsx-a11y` with SC 1.3.1** — landing it changed nothing here. That limit is why the coverage argument had to be settled by a rendered sweep rather than by a rule, and since `586712c2` it has been: the route sweep in the `routeOutline.render.test.tsx` row covers **every route**, so the per-component denominators this paragraph used to turn on no longer bound the check. They are kept because they still bound the *other two* heading checks: the first is blind to expression-built headings (**11 of 33 `<h1>` and 11 of 42 `<h2>`** were already invisible to it at `6911bcb`), and `headingOutline.ts` is asserted directly against 3 components. **Re-derive both denominators from the source, not from a raw glob, and not from this sentence** — both moved between `f3ed4e39` and `22e60707`, where they are **30 page components and 33 distinct route paths**: `packages/web/src/pages/*.tsx` is 41 files, but 11 of them are tests, and `App.tsx` declares 36 `<Route>` elements resolving to 33 distinct `path=` values, two of which (`*`, `/*`) are catch-alls. The sweep's 30 routes reconcile against that exactly — 33 declared, less the three the test excludes by name as rendering no page (`/*` the `ProtectedRoute` wrapper, `/dashboard` and `/reports/pipeline` both `<Navigate>`).
>
> **~~Four `h1` → `h3` skips are live on `main`~~ — all four are fixed, and the class is now enforced.** They were live and invisible to every check at `f3ed4e39`; `586712c2` (PR #299, WIC-1675) fixed each one and added the sweep that makes a recurrence fail the build. Re-measured at `22e60707`: `ProjectsList.tsx:145`, `ResumeManager.tsx:189`, `ProjectDetail.tsx:66` and `SavedFilterShortcuts.tsx:127` are all now `<h2>`, and `Login.tsx:63` went `<h2>` → `<h1>`. **The paragraph is kept, in the past tense, because the shape of the defect is the argument for the check that now guards it** — delete the history and the next reader has no reason the sweep renders four branches rather than one. Three shared one shape: `pages/ProjectsList.tsx` (`<h1>` `:107`, `<h3>` `:145`), `pages/ResumeManager.tsx` (`:119`, `:189`) and `pages/ProjectDetail.tsx` (`:40`, `:66` — the only one unmoved) each skipped in the *populated* render branch only, while the *empty* branch of all three was already correct because `EmptyState` gained a `headingLevel` prop under WIC-1417. **That asymmetry is why the sweep renders four branches per route and not one** — a per-route check that rendered a single branch would have reported all three clean. Line numbers in this paragraph are the `f3ed4e39` positions the skips were measured at, not current ones.
>
> **The fourth should change how the table above is read, because it is the page the rendered-outline check was written for.** `pages/ApplicationsList.tsx` skips in *every* branch: `<h1>` "Applications" at `:155`, then `SavedFilterShortcuts` — mounted unconditionally at `:194` — renders `<h3>` "Filter Shortcuts" at `SavedFilterShortcuts.tsx:124`. The `<h2>` column headings do not arrive until `KanbanBoard` is mounted at `:224`, after it. **Note the ownership, because the previous revision of this sentence got it wrong:** `KanbanBoard.tsx` contains no heading element at all — the `<h2>` is `KanbanColumn.tsx:65`, and `KanbanColumn`'s own source comment states the reasoning ("`h2`, not `h3` (WIC-1563). The only host is `KanbanBoard`, which owns no heading of its own, so this sits directly under the page `<h1>`"). That is precisely why the skip is invisible per-file: the `<h1>`, the offending `<h3>` and the `<h2>` that should sit between them live in three different files. The check stayed green because `KanbanBoard.test.tsx`'s `renderBoard` helper renders a hand-written approximation of the page — a literal `<h1>Applications</h1>` followed by `<KanbanBoard>` — and that fixture never mounts the sibling that causes the skip. **A rendered-outline assertion certifies the composition it renders, not the route it is named after.** That sentence is the durable lesson here and it is still true; what has changed is that the fixture is no longer the only thing standing between this page and a green check. Its sibling check is the control that showed this was a fixture problem rather than a method problem: `CoverLetterPreview.test.tsx` builds its fixture the same way, and there the real page does agree — `CoverLetterDetail.tsx` goes `<h1>` `:104` → `<h2>` from `CoverLetterPreview`, with no skip.
>
> **Resolved by `586712c2`, and the resolution is the reason the `routeOutline.render.test.tsx` row is worth its weight.** `routeOutlineRoutes.tsx` mounts the **real** `<ApplicationsList />`, so `/applications` is now judged as the route rather than as a fixture, and the skip described above would fail the build in all four branches. **The `KanbanBoard.test.tsx` fixture is unchanged and still does not match the page** — that was never the thing to fix, and "nothing checks that it does" is what stopped being true. WIC-1834 is closed.
>
...
>
> **Two of the seven now state the opposite of what is true.** UC-3's AC-Q3 says "nothing in the repository could have failed it, because there is no accessibility tooling at all", and UC-5's AC-N7 says "Nothing in the repository can fail AC-N7 automatically". Both were correct when written and both are now wrong in the narrow way that matters — the seven checks in the table above do fail the build, and since `f3ed4e39` two of those seven are accessibility *lint* rules, which is the exact thing UC-3 says does not exist. **The figure they should carry is now much stronger than the one this paragraph used to give, and the old one should not be requoted**: it read "the rendered-outline check reaches components inside **two** of the seven specs (UC-4's `CoverLetterPreview`, UC-5's `KanbanBoard`), and for UC-5 it reaches a fixture rather than the route." Since `586712c2` the sweep reaches **every route of every one of the seven specs**, as the route rather than as a fixture — including UC-5's `/applications`, which is the case the old sentence singled out as unreached. Amending the specs themselves is a Business Analyst edit and is routed as WIC-1833; this note is the repository-side flag required by **WIC-1584 AC-3**, and WIC-15 §8-A is its specification-side counterpart.
>
...
>
> **~~What did not land, and is the reason this is half a mechanism~~ — superseded 2026-09-01 by WIC-1926.** That paragraph said "**`axe-core` is still not a dependency** — it appears nowhere in any `package.json`, and the only mention of it anywhere in the tree is a comment in `src/test/prohibitedName.ts:10` describing what axe *would* report." **Both halves are now false**, and it is restated rather than silently deleted because it was this document's standing citation for "half a mechanism". `axe-core@^4.13.0` is now a `packages/web` devDependency; `src/test/axe.ts` is the single shared assertion Ruling 2 asked for; and `prohibitedName.ts` — the hand-rolled stand-in for `aria-prohibited-attr` — was **deleted** in the same change, after axe was confirmed to flag both components it had been written for. Neither `pa11y` nor a Lighthouse budget exists, and neither was ever ruled for.
>
> **What axe covers, as a measurement rather than a claim.** It runs over the **rendered** DOM of all **120 (route, branch) pairs** the WIC-1675 harness mounts, plus the two component tests. Two rules are excluded, both because *jsdom cannot answer them* and neither because the app is exempt: `region` (the app's `<main>` lives in `App.tsx` chrome the route harness deliberately does not mount — 354 findings, entirely harness artifact) and `color-contrast` (no layout, no canvas — 108 needs-review, 0 violations). What remains is frozen at **26 findings across 22 (rule, route, branch) keys** in `src/test/routeAxe.render.test.tsx`, on four routes: unlabelled `<select>` filters on `/resume-variants`, an unlabelled file input on `/resumes/upload`, an unnamed `role="progressbar"` on `/projects/new/dialogue`, and needs-review on Radix's own focus-guard sentinels. **Shrink-only in both directions**, like the lint baseline: a new finding fails the build, and a *fixed* one fails until its line is deleted. Retiring those 26 is **WIC-1942**, not that change.
>
> **The two layers are disjoint, and that is measured rather than asserted.** `jsx-a11y` at `strict` returns **zero** findings for `aria-label` on a role-less element, because `role-supports-aria-props` only fires once an element *has* a role — which is exactly WIC-1185 and WIC-1191. Restoring either defect now fails the suite on axe's `aria-prohibited-attr`, in both cases as **`incomplete`, not `violation`**: an element carrying text leaves axe unable to decide whether the author name or the text content wins, which is the ambiguity that made it a bug. A helper reading `violations` alone therefore reports both real defects as clean, so `src/test/axe.test.ts` pins that three-way result directly against literal fixtures.
>
...
>
> **What remains unverified.** Heading order plus a lint rule set **plus axe** is not WCAG 2.1 AA, and nothing above changes that — the ruling's own §7 says so, and adopting the second half of it does not make the pair complete. Read the section above as "the ruled mechanism is now fully landed", never as "this tree is AA". The 47 `jsx-a11y` findings are **frozen, not fixed** — that is **WIC-1589**, still open — and so are the 26 axe findings, which are **WIC-1942**. **~~Rendered per-route, per-render-branch outline coverage is WIC-1675; until it lands, the four skips documented above remain invisible to every check in the table.~~ That is superseded: WIC-1675 landed as `586712c2` (PR #299), the four skips are fixed, and the coverage is enforced with no allowlist.** What is left of that gap is narrower and specific: **25 (route, branch) pairs open at a heading level other than `<h1>`**, inventoried in `MISSING_H1` and shrink-only. **21 of the 25** are `loading` or `error` branches whose `<h1>` sits behind an early return. Those branches are **not nameless to the browser**, and have not been since `7f8646d2` (WIC-1089, PR #336): 15 of the 21 are titled by the shell's `RouteTitle` effect, which is mounted once in `App.tsx` *outside* every page's render branch and so is untouched by an early return; the other 6 are titled by pages that call `useDocumentTitle` **above** their own early return — 4 of those reading `DYNAMIC_TITLE_FALLBACKS` while the record loads, 2 titling from a URL param already known before any fetch. Measured pair by pair at `ca776dda`: all 21 carry a real `document.title`, and re-running the same sweep with `RouteTitle` unmounted leaves 15 of them showing the *previous* route's title — so the effect is load-bearing and **SC 2.4.2 (Page Titled, Level A) holds on all 21**. What is missing there is the **heading**, not the name: a screen reader building a document outline finds no `<h1>` exactly while the page is slowest and while it is reporting a failure, and a `document.title` is not a heading. That is an SC 1.3.1 / 2.4.6 defect, not a formality. The remaining 4 are the two routes that lack a route-level `<h1>` in **every** branch: `/applications/new`, whose body is a Radix dialog titled by an `<h2>`, and `/applications/:id/prep`, which opens on `<h2>` "No Interview Prep Yet" with nothing above it. **Missing an `h1` is a real SC 1.3.1/2.4.6 defect, not a formality — an allowlisted defect is still a defect**, and this list is the honest measure of what the sweep does not yet require. Citation written under **WIC-1584**; revised under **WIC-1902**; that revision measured at `22e60707` after PR #299; revised again under **WIC-1926**, and the "nameless" clause corrected under **WIC-1959** after PR #336 falsified it.
