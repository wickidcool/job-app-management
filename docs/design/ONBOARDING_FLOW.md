...
```typescript
GET  /api/users/me/onboarding/status       // current row, or null if uninitialized
POST /api/users/me/onboarding/progress     // update currentStep + step flags
POST /api/users/me/onboarding/complete     // mark completed, stamp completedAt
GET  /api/users/me/onboarding/should-show  // whether to surface the modal
```

> There is **no** `POST /api/users/me/onboarding/dismiss`. This document
> specified one; it was never implemented, and no dismissal is persisted
> anywhere. Dismissal is client-side modal state only.
