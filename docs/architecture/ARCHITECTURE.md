...
>
> ⛔ **The CI route to that check is blocked — the credential it depends on is revoked.** `.github/workflows/careerpin-redirect-ownership-probe.yml` (read-only, GETs only) is written and correct, and it is still the intended way to answer this: it reports whether the Worker exists and whether it holds routes on the three hostnames. But it spends `CLOUDFLARE_CAREERPIN_API`, and that token is **revoked or expired** — measured 2026-09-05, run `33983093085`: `GET /accounts` **403 / Cloudflare code 9109**, and the zone-read control **403 / 9109** too, so it cannot even read its own zone. Dispatching the probe today buys a red run and no answer. The pending rotation ask is **WIC-2100**; do not file another. **The question itself is still open and still tracked by WIC-2111** — what is unavailable is this route to answering it, not the check. Until the token is rotated, the only remaining route is a dashboard session. (An earlier revision of this line claimed the check "no longer needs a dashboard session", on a credential that was alive when written. `CLOUDFLARE_API_TOKEN` is a *different* secret and is alive — see [CREDENTIAL_REGISTRY.md](./CREDENTIAL_REGISTRY.md); whether it can take this workflow over is WIC-2118's question, not an assumption to act on here.)
>
