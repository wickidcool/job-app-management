...
| Field | Unit | Status |
|-------|------|--------|
| `PrepStory.relevanceScorePct` (interview prep) | `[0, 100]` integer | Persisted as `integer` (`schema.ts:708`) and generated by a prompt asking for a 0-100 integer, so ADR-008 §4 renames it rather than converting it. **The wire field is still `relevanceScore` on `origin/main`** — the rename is WIC-1516's backend/frontend children. |
...
interface PrepStory {
  ...
  relevanceScorePct: number;          // Percent in [0,100], integer — match to job requirements. Deviates from the ratio convention by ADR-008 §4; the `Pct` suffix is why. Wire field is still `relevanceScore` until the rename lands
