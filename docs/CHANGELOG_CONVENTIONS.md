...
> Extracted from `CLAUDE.md` (WIC-2395). This content is unchanged and remains authoritative.
> It lives here rather than in `CLAUDE.md` because `CLAUDE.md` is loaded into **every agent's
> context on every turn**, and this section was 77% of that file. Read it before editing
> `CHANGELOG.md` or resolving a changelog merge.
